...
export const projects: Project[] = [
  ...
  // ── 02 · 프로필 서비스 MFE 이관과 신규 개편 ─────────────────────────────────
  {
    ...
    problem: {
      lead: '닉네임만 보여주던 프로필에 투자의견·거래내역·포트폴리오 같은 신규 스펙이 추가되면서, 카카오페이 인프라에 묶여 있던 배포 체계부터 증권 전용 MFE 인프라로 옮겨야 했습니다.',
      points: [
        '이관과 신규 개편이 같은 빠듯한 일정에 묶여 있었는데, 신규 환경의 공통 패키지를 고치려면 의존하는 모든 팀의 승인을 받아야 했습니다. 실제로 4월에 공유된 CI 속도 개선 코드가 다른 팀들의 검증을 거쳐 반영되기까지 2개월이 걸렸을 정도로, 승인 대기 기간을 예측할 수 없었습니다.',
        '동시에 서버 개발도 이관 작업과 겹쳐 있어, 확정된 API 명세 없이 기획 초안만 보고 프론트엔드를 먼저 만들어야 했습니다.',
      ],
    },
    decision: {
      lead: 'CI 코드 하나가 반영되기까지 2개월이 걸리는 구조로는, 이관과 개편을 같은 일정 안에서 끝낼 수 없었습니다.',
      points: [
    ...
    result: {
      ...
      points: [
        ...
        '"내 프로필" 진입 API 응답 시간은 개발자도구 네트워크 탭 실측 기준 약 180ms 줄었습니다. 응답 전까지 스켈레톤을 보여주는 구조라, 화면이 완성되는 시점의 단축과도 같습니다.',
      ],
    },
    flow: {
      problem: '신규 스펙 추가로 증권 전용 MFE 이관 필요, 이관과 개발이 한 일정에 충돌',
  ...
  // ── 03 · 프로필 접근 제어 모듈 설계 ──────────────────────────────────────────
  {
    ...
    result: {
      ...
      points: [
        ...
        '다만 60초 staleTime 동안은 탭을 전환해도 변경된 접근 조건이 곧바로 반영되지 않는 한계가 남았습니다. 캐시를 완전히 비우는 pull-to-refresh로 해소할 수는 있지만, 탭 전환 정도의 가벼운 액션에 캐시 전체를 초기화하는 건 과한 대응이라 더 가벼운 해결책을 찾고 있습니다.',
      ],
    },
    flow: {
