...
export const projects: Project[] = [
  ...
  // ── 02 · 프로필 서비스 MFE 이관과 신규 개편 ─────────────────────────────────
  {
    ...
    problem: {
      ...
      points: [
        '이관과 신규 개편이 같은 빠듯한 일정에 묶여 있었는데, 신규 환경의 공통 패키지를 고치려면 의존하는 모든 팀의 승인을 받아야 해서 상수 하나 추가하는 데도 배포 사이클을 통째로 놓치곤 했습니다.',
        '동시에 서버 개발도 이관 작업과 겹쳐 있어, 확정된 API 명세 없이 기획 초안만 보고 프론트엔드를 먼저 만들어야 했습니다.',
      ],
    },
    decision: {
      lead: '상수 하나를 추가하는 데도 배포 사이클을 통째로 날리는 구조로는, 이관과 개편을 같은 일정 안에서 끝낼 수 없었습니다.',
      points: [
        ...
        '한편 API 명세가 확정되기 전이라, 기획 초안을 그대로 MSW 핸들러로 구현해 실제 로직을 태워보며 명세의 허점을 미리 찾았습니다. 그중 "내 프로필" 진입 시 본인 userId를 몰라 발생하던 불필요한 직렬 API 호출은, 클라이언트 대신 서버가 세션으로 사용자를 식별하는 전용 엔드포인트를 요청해 없앴습니다.',
      ],
    },
    result: {
      lead: '패키지를 팀 안에 격리한 절충안과, 서버 명세를 기다리지 않고 먼저 검증한 접근 모두 결과로 이어졌습니다.',
      points: [
        '공통 패키지 승인 없이 팀 내부 검토만으로 즉시 배포할 수 있게 되어, 매주 배포 사이클마다 반복되던 지연이 사라졌습니다.',
        '"내 프로필" 진입 API 응답 시간은 약 180ms 줄었습니다. 두 선택 모두 완벽한 정답은 아니었지만, 빠듯한 일정 안에서 통제 가능한 리스크를 고른 결과였습니다.',
      ],
    },
    flow: {
  ...
  // ── 03 · 프로필 접근 제어 모듈 설계 ──────────────────────────────────────────
  {
    ...
    problem: {
      ...
      points: [
        '기존엔 각 컴포넌트가 이 조건들을 if 분기로 직접 처리했는데, 이미 데이터 로딩과 콜드 케이스 분기가 있는 상태라 권한 분기까지 더해지며 한 컴포넌트가 감당해야 할 책임이 한꺼번에 늘어나는 구조가 됐습니다.',
        '정책이 바뀌면 권한 조건이 관여하는 모든 컴포넌트를 찾아 고쳐야 했고, 수익률처럼 더 민감한 정보를 요구하는 기능이 추가될 예정이라 조건은 앞으로도 늘어날 상황이었습니다.',
      ],
    },
    decision: {
      lead: '조건이 계속 늘어날 상황에서, 컴포넌트는 "무엇을 보여줄지"만 알고 "왜 못 보여주는지"는 몰라도 되도록 책임을 나누는 쪽으로 방향을 잡았습니다.',
      points: [
  ...
  // ── 04 · 코드 리뷰 생산성 크롬 익스텐션 ──────────────────────────────────────────
  {
    ...
    decision: {
      lead: '가시성 문제는 목록을 한눈에 모아 보여주는 대시보드 화면으로 비교적 단순하게 풀렸지만, 실시간 알림에서는 같은 원인의 기술적 문제를 두 번 만났습니다.',
      points: [
  ...
  // ── 05 · responsive-keepalive ─ 반응형 트리 전환 상태 보존 라이브러리 ──────────────────────────────────────────
  {
    ...
    decision: {
      ...
      points: [
        ...
        '그 위에 문제 크기별로 API 3종을 나눴습니다 — 트리는 useMediaVariant, 카드는 useContainerVariant, 값 하나는 useResponsiveValue입니다. 라이브러리 배포 특성상 Activity 미지원 환경도 최소 동작은 보장해야 했습니다.',
      ],
    },
    result: {
      lead: '그 전제를 지키며, 런타임 의존성 없이 npm에 배포했습니다.',
      points: [
        'SSR 환경에서는 서버에 없는 window 대신 설정된 ssr variant를 반환하고 하이드레이션 후 한 번만 재조정했고, React 19.2 미만에서는 상태 보존 없는 swap으로 자동 폴백했습니다. 97개 테스트, 98.6% 라인 커버리지로 완성도를 확인했습니다.',
      ],
    },
    flow: {
    ...
    highlights: [
      {
        ...
        tone: 'tech',
        problem:
          '핵심 기능인 코드 에디터(CodeMirror) 라이브러리 용량이 커서, 첫 화면 로드가 눈에 띄게 무거웠습니다.',
        solution:
          'bundle-analyzer로 뜯어보니 실제로 쓰지 않는 언어 문법까지 전부 포함돼 있었습니다. 지원 언어를 실사용분으로 좁혀 Tree Shaking이 걸리게 하고, 에디터는 Dynamic Import로 초기 번들에서 떼어내 2.2MB에서 1.4MB로 줄였습니다.',
      },
      {
        label: '디자인 시스템',
        tone: 'tech',
        ...
        solution:
          '구조를 읽을 수 없는 "Flex Soup"과 Emotion의 의미 있는 네이밍 상실을 겪은 뒤로는, 무조건적인 세분화 대신 기술 스택 특성에 맞춰 추상화 수준을 판단하는 기준으로 컴포넌트를 설계하고 있습니다.',
      },
      {
        label: '웹 접근성',
        tone: 'tech',
